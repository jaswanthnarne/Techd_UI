...
const CTFSubmission = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!flag.trim()) {
      ...
      return;
    } else if (flag.trim().length < 3) {
      toast.error("Flag must be at least 3 characters");
      return;
    }
  ...
  return (
    <StudentLayout title={ctf.title} subtitle="Submit mission evidence">
      ...
      <Modal
        ...
        onClose={() => {
          setShowEditModal(false);
          setFlag(""); // Reset flag when closing
        }}
        ...
        <form onSubmit={handleEdit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setFlag(e.target.value)}
              placeholder="CTF{...}"
              className={`block w-full border-2 rounded-xl shadow-sm px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-lg font-mono `}
              required
              disabled={!canSubmit}
            />
          </div>
